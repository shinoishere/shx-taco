...
function TeslimatBasla()
    ...
    TriggerEvent('notification', 'Green taco dağıtımı başladın!', 1, 6500)
    ...
    Citizen.Wait(10)
        for k,v in pairs(EtPisir) do
            ...
            if not inMission then
               DrawText3D(v.x, v.y, v.z+0.15, '~g~E~w~ - Taco Pisir')
...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(10)
        for k,v in pairs(TacoHazirlamak) do
            ...
            if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3)  then
            if not inMission then
               DrawText3D(v.x, v.y, v.z+0.15, '~g~E~w~ - Taco Hazırla')
               if IsControlJustReleased(0, 38) then
                    hangitacoyapsash1no()
                        end
                    end
...
function hangitacoyapsash1no()
    ...
    { label = 'Balıklı Taco', value = 'ananasokayim'},
	
	}
	
	ESX.UI.Menu.CloseAll() 
	ESX.UI.Menu.Open('default', GetCurrentResourceName(),'ventacyc',{
		title = 'Hangi taco yapmak istiyosun?', 
		align = 'top-right',
		elements = elements  
...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(10)
        for k,v in pairs(TacoPaketleme) do
            ...
            if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3)  then
            if not inMission then
               DrawText3D(v.x, v.y, v.z+0.15, '~g~E~w~ - Taco Paketleme')
               if IsControlJustReleased(0, 38) then
                exports['mythic_progbar']:Progress({
                    name = "taco_paketleme",
                    duration = 20000,
                    label = 'TACO PAKETLIYORSUN',
                    useWhileDead = false,
                    canCancel = false,
                    controlDisables = {
                        disableMovement = true,
		...
		for k,v in pairs(TacoSatis) do
            ...
            if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3)  then
				if not inMission then
					...
					DrawText3D(v.x, v.y, v.z+0.15, '~g~E~w~ - Teslimati iptal et')		
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
						SetPedComponentVariation(ped, 5, 20, 0, 0)
						inMission = false
						TriggerEvent('notification', 'Green taco dağıtımı iptal ettin!', 2, 6000)
					    end
				    end
                end
            end
...
Citizen.CreateThread(function()
    ...
    DrawText3D(coords[1], coords[2], coords[3], '~g~E~w~ - Kapıyı Çal')
